...
export class FloorService {
  ...
  constructor(
    ...
    private _floorMaps: FloorMapsService,
    private _buildings: BuildingsService,
    private _reservationMarks: ReservationMarksService,
    ...
    this._floors$ = this._route.params.pipe(
      switchMap((params) => {
        ...
        return combineLatest([
          this._floorMaps.getFloorMaps$(itemType),
          this._buildings.getBuildingsByPlaceType$(itemType),
          of(+params?.floorId),
        ]);
      }),
    ...
    this.reservations$ = combineLatest([this.currentFloor$, this.datesFilter$, this.currentItems$, this._reload$]).pipe(
      ...
      switchMap(([{ id }, { dateTimeFrom, dateTimeTo }, items]) => {
        ...
        switch (this._route.parent.snapshot.params.type) {
          case 'appointment':
            params = {
        ...
        return this._api.getMapReservationsByType(this._route.parent.snapshot.params.type, params);
      }),
      shareReplay(1),
    );
  }
  ...
  get itemsWithFilters$() {
    return combineLatest([
      ...
      this.currentFloor$,
      this._reservationMarks.mapMarks$,
    ]);
  }
  ...
  get mapMarks$(): Observable<IMarks> {
    return this.itemsWithFilters$.pipe(
      ...
      map(({ places, images, reservationMarks }) => {
        return [
          ...mapPlacesToMArks(places, images, this._route.parent.snapshot.params.type),
  ...
  get filterDates$(): Observable<Record<string, string>> {
    return this._filterService.filters$.pipe(
      switchMap((filters) =>
  ...
  get filterDatesClean$(): Observable<Record<string, string>> {
    return this._filterService.filters$.pipe(
      map((filters) => ({
      ...
      switchMap(({ repeat, dateTimeFrom, dateTimeTo }) => {
        if (repeat) {
          return this.currentFloor$.pipe(
            switchMap(({ id }) => {
              return this._api
                .getReservationsByType(this._route.parent.snapshot.params.type, {
                  floorMapId: id,
                  dateTimeFrom,
    ...
    return this._filterService.filters$
      .pipe(
        ...
        switchMap(([filters, repeat]) => {
          const reservations = [
            this._setReservationByType(this._route.parent.snapshot.params.type, 'main', {
              id,
          ...
          if (repeat && repeat[0] && repeat[0].length) {
            repeat[0].forEach((_dates, index) => {
              reservations.push(
                this._setReservationByType(this._route.parent.snapshot.params.type, index, {
                  id,
          ...
          return this._api.putReservationsByType(this._route.parent.snapshot.params.type, params).pipe(
            map(({ data, error }) => {
              return {
                data,
  ...
  private _getApiDate(date: Date): string {
    return this._datePipe.transform(date, this._measurements.getMeasurementByName('filterDate'));
  }
  ...
  private _checkFilters(places, reservations, tags, repeat, types, seatCount): IWorkplace[] {
    ...
    filteredPlaces = filteredPlaces.map((place) => {
      ...
      if (reservations?.length && this._route.parent.snapshot.params.type === 'appointment') {
        const reservation = reservations?.find((r) => r.roomId === place.id);
        newPlace.reservation = reservation;
      }
      if (
        reservations?.length &&
        (this._route.parent.snapshot.params.type === 'workplace' || this._route.parent.snapshot.params.type === 'parking')
      ) {
        const reservation =
          this._route.parent.snapshot.params.type === 'parking'
      ...
      if (this._route.parent.snapshot.params.type === 'appointment') {
  ...
  setFiltersDate(filters: Record<string, any>): void {
    this._reservationMarks.setDateFilterValue([filters.dateTimeFrom, filters.dateTimeTo]);
  }

  get type() {
    return this._route.parent.snapshot.params.type;
  }
}
